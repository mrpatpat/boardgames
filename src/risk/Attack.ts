...
export class Attack extends Action<IRiskState, RiskPlayer> {
    constructor(
        origin: RiskPlayer,
    ...
    transform(state: IRiskState): IRiskState {
        ...
        if (origin && target) {
            console.log(
                `${this.origin.name} attacks ${target.soldiers} in ${
                    target.name
                } with ${origin.soldiers} from ${origin.name}`
            );
            ...
            if (origin.soldiers === target.soldiers) {
                ...
                console.log(
                    `${this.origin.name} occupied ${target.name} with ${
                        target.soldiers
                    } in ${target.name} and ${origin.soldiers} in ${
                        origin.name
                    }`
                );
            } else if (origin.soldiers < target.soldiers) {
                origin.soldiers = 1;
                console.log(
                    `${this.origin.name} lost all but 1 soldier in ${
                        origin.name
                    }`
                );
            }
