...
export class Deploy extends Action<IRiskState, RiskPlayer> {
    ...
    transform(state: IRiskState): IRiskState {
        ...
        const amount = state.countries.filter(c => c.occupist === this.origin)
            .length;

        if (country) {
            country.soldiers += amount;
            console.log(
                `${this.origin.name} deploys ${amount} soldiers to ${
                    country.name
                }`
            );
        }
