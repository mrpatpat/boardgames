...
export class SelectStarterCountry extends Action<IRiskState> {
    ...
    transform(state: IRiskState): IRiskState {
        ...
        if(country) {
            const p = this.origin as RiskPlayer;
            p.selectedStarterCountry = true;
            country.occupist = p;
            country.soldiers = 5;
            console.log(`${this.origin.name} selected ${country.name} as starting point with ${country.soldiers} soldiers`);
        }
        return {...state, turn: state.turn + 1, currentPlayerIndex: (state.turn + 1) % state.players.length};
    }
