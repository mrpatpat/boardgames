...
export function isCurrentPlayersAction(action: Action<IRiskState>, state: IRiskState): boolean {
...
export function isOccupiedByActionPlayer(action: Action<IRiskState>, state: IRiskState, countryId: number): boolean {
    const country = state.countries.find(c=>c.id === countryId);
    return country ? country.occupist === action.origin : false; 
}

export function traversable(state: IRiskState, originId: number, targetId: number): boolean {
